Tutorial bezüglich Views und Routing.
https://scotch.io/tutorials/single-page-apps-with-angularjs-routing-and-templating
Angular injected den Content der htmls aus pages in die index html --> globale scripts und css files muessen dort deklariert werden

clean URL get rid of #
https://scotch.io/quick-tips/pretty-urls-in-angularjs-removing-the-hashtag


Bootstrap NAVBAR
http://getbootstrap.com/components/#navbar

home und alle Auftraege unterscheiden sich nur anhand der Abfrage

TODO:
DropDown Menü anpassen bei buchung erstellen --> bootstrap 
http://stackoverflow.com/questions/15090185/angularjs-bootstrap-dropdown-cant-do-ng-click-in-ng-repeat

Links hinter APs beim Anzeigen

Sichtbarkeitssteuerung bug. immer nur erste, id muss vom ng model kommen nicht dynamisch angepasst

tagesdatum bei buchung erstellen


send data to rest 
http://stackoverflow.com/questions/17211443/change-content-type-to-application-json-post-method-restful-api

java seite 
http://hello-angularjs.appspot.com/angularjs-http-service-ajax-post-json-data-code-example

aufklappbares menü
https://angular-ui.github.io/bootstrap/#/accordion

ermittleMeineLetztenBebuchtenArbeitspakete/{sbNr}
buchen mit JSON Object:
kommentar
arbeitspaket
buchungsErsteller
aufwand
https://webservices-test.badenia.de:8085/BadeniaRochadeRESTServices/zeiterfassung/ermittleAuftraege/1472

HTTP Header Accept
application/json

Authentifizierung
http://www.jasonwatmore.com/post/2014/05/26/AngularJS-Basic-HTTP-Authentication-Example.aspx
https://docs.angularjs.org/api/ng/service/$http#jsonp
http://plnkr.co/edit/H4SVl6?p=preview

Navigation:
http://stackoverflow.com/questions/25320162/hiding-certain-html-elements-when-using-ng-view-angularjs

Sortieren der tabelle
https://scotch.io/tutorials/sort-and-filter-a-table-using-angular


Service umsortieren

checkbox tabelle
http://stackoverflow.com/questions/11872832/how-to-respond-to-clicks-on-a-checkbox-in-an-angularjs-directive
http://stackoverflow.com/questions/19590063/get-specific-json-object-by-id-from-json-array-in-angularjs
http://stackoverflow.com/questions/18884840/adding-a-new-array-element-to-a-json-object
http://stackoverflow.com/questions/6310623/remove-item-from-json-array-using-its-name-value

Clickable table row fuer mitarbeiter

Loading bar http 
https://github.com/chieffancypants/angular-loading-bar
