	...
	$sceProvider.enabled(false);
		...
		[
				...
				function($scope, $http, $rootScope, $log, $timeout) {
					...
					/**
					 * ermittelt die Konfiguration fuer das Reporting diese
					 * binhaltet welche Berichte erlaubt werden fuer den User
					 * sowie die Daten die fuer die Berichte benoetigt werden
					 */
					...
					/**
					 * User fuer den der Bericht ausgefuehrt wird. Nicht immer
					 * gleich der angemeldete User --> Gruppenleitung.
					 */
					$scope.reportingUser = {
						"name" : $rootScope.user,
					...
					/**
					 * Downloaded den ausgewaehlten Report abhaengig von
					...
					$scope.downloadReport = function() {
						...
						if ($scope.selectedReport == 'aufwandNachAuftragArbeitspaket') {
							...
							$log.debug("downloadReport: " + serviceURL + "/report/" + "aufwandNachAuftragArbeitspaket" + "/" + $scope.reportingUser.name + "/" + germanDateFormatter($scope.beginnDatum.value)
									+ "/" + germanDateFormatter($scope.endDatum.value));

							$http.get(
									serviceURL + "/report/" + "aufwandNachAuftragArbeitspaket" + "/" + $scope.reportingUser.name + "/" + germanDateFormatter($scope.beginnDatum.value) + "/"
											+ germanDateFormatter($scope.endDatum.value), {
									...
									function(data) {
										...
										if (window.navigator && window.navigator.msSaveOrOpenBlob) {
											window.navigator.msSaveOrOpenBlob(file, "Mitarbeiterbericht-" + $scope.reportingUser.name + "-" + germanDateFormatter($scope.beginnDatum.value) + "-"
													+ germanDateFormatter($scope.endDatum.value) + ".pdf");
										} else {
											var objectUrl = URL.createObjectURL(file);

											a.href = objectUrl;
											a.download = ("Mitarbeiterbericht-" + $scope.reportingUser.name + "-" + germanDateFormatter($scope.beginnDatum.value) + "-"
													+ germanDateFormatter($scope.endDatum.value) + ".pdf");
						...
						if ($scope.selectedReport == 'aufwandNachAuftragMitarbeiter') {
							$log.debug("ausgewaehlter Bericht: " + $scope.selectedReport);
							$log.debug(serviceURL + "/report/" + "aufwandNachAuftragMitarbeiter" + "/" + $scope.selectedAuftrag.name + "/" + germanDateFormatter($scope.beginnDatum.value) + "/"
									+ germanDateFormatter($scope.endDatum.value));

							$http.get(
									serviceURL + "/report/" + "aufwandNachAuftragMitarbeiter" + "/" + $scope.selectedAuftrag.name + "/" + germanDateFormatter($scope.beginnDatum.value) + "/"
											+ germanDateFormatter($scope.endDatum.value), {
										responseType : 'arraybuffer'
									}).success(
									function(data) {
										...
										if (window.navigator && window.navigator.msSaveOrOpenBlob) {
											window.navigator.msSaveOrOpenBlob(file, "Bericht-" + $scope.selectedAuftrag.name + "-" + germanDateFormatter($scope.beginnDatum.value) + "-"
													+ germanDateFormatter($scope.endDatum.value) + ".pdf");
										} else {
											var objectUrl = URL.createObjectURL(file);

											a.href = objectUrl;
											a.download = "Bericht-" + $scope.selectedAuftrag.name + "-" + germanDateFormatter($scope.beginnDatum.value) + "-" + germanDateFormatter($scope.endDatum.value)
													+ ".pdf";
											$timeout(function() {
												a.click();
											})
